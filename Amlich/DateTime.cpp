...
void DateTime::shiftData(int dataPin, int clockPin, int slatchPin) {
	
	unsigned char Tens, Units;
	Tens = getYear() / 10;
	Units = getYear() % 10;
	shiftOut(dataPin, clockPin, MSBFIRST, LEDCode[Units]);
	shiftOut(dataPin, clockPin, MSBFIRST, LEDCode[Tens]);
	shiftOut(dataPin, clockPin, MSBFIRST, LEDCode[0]);
	shiftOut(dataPin, clockPin, MSBFIRST, LEDCode[2]);
	Tens = getLunarMonth() / 10;
	Units = getLunarMonth() % 10;
    if (isLeafMonth()){
        shiftOut(dataPin, clockPin, MSBFIRST, LEDCode[Units] - 128);      
    }  
    else{
        shiftOut(dataPin, clockPin, MSBFIRST, LEDCode[Units]);
    }
    if(getDay() == 7){
        shiftOut(dataPin, clockPin, MSBFIRST, LEDCode[Tens] - 128);
    }
    else{
        shiftOut(dataPin, clockPin, MSBFIRST, LEDCode[Tens]);
    }
	Tens = getLunarDate() / 10;
	Units = getLunarDate() % 10;
    if(getDay() == 6){
	    shiftOut(dataPin, clockPin, MSBFIRST, LEDCode[Units] - 128);
    }
    else{
...
Date Date::operator+(const unsigned char& b) {
	Date *addedDate = new Date(date + b, month, year);
...
unsigned char GetMaxDay(unsigned char lunarMonth, unsigned char byte1, unsigned char byte2, unsigned char byte3) {
	...
	if (lunarMonth == 0) return 0;
	else if (lunarMonth == 1) GetValue = byte3 & 0x01;
	else if (lunarMonth == 2) GetValue = byte3 & 0x02;
	else if (lunarMonth == 3) GetValue = byte3 & 0x04;
	else if (lunarMonth == 4) GetValue = byte3 & 0x08;
	else if (lunarMonth == 5) GetValue = byte3 & 0x10;
	else if (lunarMonth == 6) GetValue = byte3 & 0x20;
	else if (lunarMonth == 7) GetValue = byte3 & 0x40;
	else if (lunarMonth == 8) GetValue = byte3 & 0x80;
	else if (lunarMonth == 9) GetValue = byte2 & 0x01;
	else if (lunarMonth == 10) GetValue = byte2 & 0x02;
	else if (lunarMonth == 11) GetValue = byte2 & 0x04;
	else if (lunarMonth == 12) GetValue = byte2 & 0x08;
	else if (lunarMonth == 13) GetValue = byte1 & 0x02; //leap month
...
Date Date::getLunarDateMonth() {
	...
	*/
	...
	if (year < roottime) return  *(new Date(date, month, year));
	// Get Lut 3 bytes
	...
	sD = byte1 >> 3;
	if ((byte1 & 0x04) == 0) sM = 1;
	else sM = 2;
	Date cmpDate = *(new Date(sD, sM, year));
	//Get LUT for previous year if this Start Day < Current Day
	if (*this < cmpDate) {
		if (year < 17) return  *(new Date(date, month, year));
